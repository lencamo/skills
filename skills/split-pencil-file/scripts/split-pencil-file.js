...
function printUsage() {
  console.log(`Usage:
  node split-pencil-file.js --input <source.pen> [--output-dir <dir>] [--page <name-or-index>] [--force] [--dry-run]
...
Options:
  ...
  --dry-run            Print split plan without writing files
  --help               Show this help
`);
}
...
function selectContainer(doc, pageArg) {
  if (Array.isArray(doc.pages)) {
    if (!pageArg) {
      const pages = doc.pages.map((page, index) => `${index}: ${page.name || '(unnamed)'}`).join('\n');
    ...
    const page = pageIndex >= 0
      ? doc.pages[pageIndex]
      : doc.pages.find((item) => item.name === pageArg);
...
function main() {
  ...
  const frames = topLevelNodes.filter((node) => node && node.type === 'frame');
  ...
  for (const item of manifest) {
    ...
    if (fs.existsSync(frameDir)) {
      if (!args.force) {
        throw new Error(`Output directory already exists: ${frameDir}. Re-run with --force to overwrite.`);
      }
